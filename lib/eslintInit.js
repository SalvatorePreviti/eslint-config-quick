...
function eslintInitAsync() {
  ...
  return loadJsonAsync(path.join(rootPath, 'package.json'))
    ...
    .then(() => {
      return copyFileIfNotExistsAsync(path.join(moduleRoot, '.editorconfig'), path.join(rootPath, '.editorconfig'))
    })
    .then(() => {
      return copyFileIfNotExistsAsync(path.join(moduleRoot, '.eslintignore'), path.join(rootPath, '.eslintignore'))
    })
}
